    ...
    <th class="pl-0 border-0" scope="row">
        <div class="media align-items-center">
            <a class="reset-anchor d-block animsition-link" href="{{route('frontend.product', $cartItem->model->slug)}}">
                <img src="{{asset('uploads/products/'. $cartItem->model->firstMedia->file_name)}}" alt="..." width="70"/>
            </a>
    ...
    <td class="align-middle border-0">
        <div class="border d-flex align-items-center justify-content-between px-3"><span class="small text-uppercase text-gray headings-font-family">Quantity</span>
            <div class="quantity">
                ...
                <span class="form-control form-control-sm border-0 shadow-0 px-2 text-center">{{ $item_quantity }}</span>
                {{-- <input class="form-control form-control-sm border-0 shadow-0 p-0" type="text" value="{{ $cartItem->qty }}"/> --}}
                <button wire:click.prevent="increaseQuantity('{{ $cartItem->rowId }}')" class="p-0"><i class="fas fa-caret-right"></i></button>
            </div>
        </div>
    </td>
    <td class="align-middle border-0">
        <p class="mb-0 small">${{$cartItem->qty * $cartItem->model->price}}</p>
    </td>
    <td class="align-middle border-0">
        <a wire:click.prevent="deleteItem('{{ $cartItem->rowId }}')" x-on:click="show = false" class="reset-anchor">
